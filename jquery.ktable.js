$.fn.ktable = function( options ) {
   ...
   * 
   * This plugin builds a HTML table and populate it with JSON retrieved from an ajax request.
   ...
   *   @param css_class_table      [string]    CSS class name for the table. Defaults to "ktable" if not set.
   *   
   ...
   *                                           
   * 
   * The HTML structure of the final table:
   * 
   * <table>
   *   <thead>
   *     <tr>
  ...
  /** Define variables for this scope level
   */

  var opts = $.extend( {}, $.fn.ktable.defaults, options ),   /*!< Object of options, including default options */
    _this = $(this),                                          /*!< Clone of $(this), to access it from sub functions */
    _data = {},                                               /*!< Clone of data, to access it from sub functions */
    table = $("<table></table>"),
    thead = $("<thead></thead>"),
    tbody = $("<tbody></tbody>"),
    thead_tr = $("<tr></tr>"),
    sort_by = "",                                             /*!< Which column to sort the table by */
    sort_desc = false;                                        /*!< If the sorting should be descending */
    first_column = "";                                        /*!< Will be set to the id of the first column in the JSON object */
  ...
  // If no sort_by is set (no column marked with "sorted"), the first column will be used:
  if ( sort_by == "" )
    sort_by = first_column;
  
  // 3Assemble the HTML elements:
  thead.append(thead_tr);
  table.append(thead);
  table.append(tbody);
  
  // Add CSS class to the table. If defined in settings.css_class_table, use that, otherwise use hardcoded "ktable":
  var table_class = ( opts.settings.css_class_table !== undefined ) ? opts.settings.css_class_table : "ktable";
  ...
  $.getJSON(opts.source, function( data ) {
    if ( data.responsecode == 200 ) { // We only accept returncode 200
      console.debug("jsonapi response: " + data.responsecode + " (" + data.response + ")");
      _data = data;     // Clone the object to, for the next functions to access it
      buildHeaders();   // Build the table
      buildTbody();     // Build the table
    } else {
      // Could not retrieve the JSON. This could be due to no response, invalid JSON, missing returncode or the returncode isn't 200
